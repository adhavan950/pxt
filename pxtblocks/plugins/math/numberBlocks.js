...
const MATH_NUMBER_MINMAX_MIXIN = {
    ...
    mutationToDom: function () {
        const slider = this.inputList[0].fieldRow[0];
        let container = Blockly.utils.xml.createElement('mutation');
        if (slider.hasMin())
            container.setAttribute('min', slider.getMin() + "");
        if (slider.hasMax())
            container.setAttribute('max', slider.getMax() + "");
        if (slider.getLabel() != undefined)
            container.setAttribute('label', slider.getLabel());
        if (slider.getStep() != undefined)
            container.setAttribute('step', slider.getStep() + "");
        // if (slider.sliderColor_ != undefined) container.setAttribute('color', slider.sliderColor_);
        if (slider.getPrecision() != undefined)
            container.setAttribute('precision', slider.getPrecision() + "");
        return container;
    },
    /**
