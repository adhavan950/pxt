...
class FlyoutButton extends Blockly.FlyoutButton {
    constructor(workspace, targetWorkspace, json, isFlyoutLabel) {
        ...
        if (!isFlyoutLabel) {
            ...
            const shadow = svgGroup.getElementsByClassName("blocklyFlyoutButtonShadow").item(0);
            const rect = svgGroup.getElementsByClassName("blocklyFlyoutButtonBackground").item(0);
            const text = svgGroup.getElementsByTagName("text").item(0);
            // resize all elements
            ...
            text.setAttribute('y', String(parseFloat(text.getAttribute("y")) - (prevHeight / 2) + (this.height / 2)));
            return;
        }
        ...
        if (icon || iconClass) {
            ...
            const svgIcon = Blockly.utils.dom.createSvgElement('text', {
                'class': `blocklyFlyoutLabelIcon${iconClass ? ' ' + iconClass : ''}${extraIconClass ? ' ' + extraIconClass : ''}`,
                'x': 0, 'y': 0, 'text-anchor': 'start'
            }, svgGroup);
            if (icon)
                svgIcon.textContent = icon;
            if (iconColor)
                svgIcon.setAttribute('style', 'fill: ' + iconColor);
            ...
            const rect = svgGroup.getElementsByClassName("blocklyFlyoutLabelBackground").item(0);
            rect.setAttribute('width', String(this.width));
        ...
        if (line) {
            const svgLine = Blockly.utils.dom.createSvgElement('line', {
