...
const collapsedInputRow_1 = require("./collapsedInputRow");
/**
 ...
 */
class RenderInfo extends Blockly.zelos.RenderInfo {
    measure() {
        if (this.block_) {
    ...
    createRows_() {
        ...
        if (this.isCollapsed) {
            ...
            if (hasStatement) {
                ...
                activeRow.elements.push(new Blockly.blockRendering.JaggedEdge(this.constants_));
            }
        }
    ...
    populateBottomRow_() {
        ...
        const followsStatement = this.block_.inputList.length &&
            ...
            || this.rows.some(row => (0, collapsedInputRow_1.isCollapsedInputRow)(row));
        ...
        if (this.bottomRow.hasNextConnection) {
            this.bottomRow.connection = new Blockly.blockRendering.NextConnection(this.constants_, this.block_.nextConnection);
    ...
    addCollapsedRow_(activeRow) {
        this.rows.push(activeRow);
        const newRow = new collapsedInputRow_1.CollapsedInputRow(this.constants_);
        // this.inputRowNum_ ++;
