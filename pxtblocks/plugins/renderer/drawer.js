...
class Drawer extends Blockly.zelos.Drawer {
    drawCollapsedStack_(row) {
        // Where to start drawing the notch, which is on the right side in LTR.
        const x = this.constants_.STATEMENT_INPUT_NOTCH_OFFSET
            + this.constants_.INSIDE_CORNERS.width;
        const innerTopLeftCorner = (this.constants_.STATEMENT_INPUT_PADDING_LEFT +
            this.constants_.INSIDE_CORNERS.width * 2) +
            Blockly.utils.svgPaths.lineOnAxis('h', -this.constants_.INSIDE_CORNERS.width) +
    ...
    drawOutline_() {
        if (this.info_.outputConnection &&
            ...
            for (let r = 1; r < this.info_.rows.length - 1; r++) {
                ...
                else if ((0, collapsedInputRow_1.isCollapsedInputRow)(row)) {
    ...
    drawLeft_() {
        ...
        const hasCollapsedStack = this.info_.rows.find(function (el) {
            return (0, collapsedInputRow_1.isCollapsedInputRow)(el);
        });
